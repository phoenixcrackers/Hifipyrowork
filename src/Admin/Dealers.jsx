    ...
    <form onSubmit={(e) => handleSubmit(e, isEdit)} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className="mt-1 block w-full text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-indigo-50 file:text-indigo-600 hover:file:bg-indigo-100"
        />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-6 flex items-center gap-x-4">
            <div>
              ...
              <input
                ...
                readOnly
                className="mt-2 w-48 rounded-md bg-gray-100 px-3 py-1.5 text-base text-gray-900 outline-none"
              />
            </div>
            <button
              onClick={() => openModal()}
              className="mt-7 bg-indigo-600 text-white px-3 py-1.5 rounded-md flex items-center gap-x-2 hover:bg-indigo-700"
            >
              <FaPlus className="h-4 w-4" /> Add Product
            </button>
          </div>
          {currentProducts.length === 0 ? (
            <p className="text-lg text-center text-gray-600 font-medium">No products found</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentProducts.map(product => {
                const productKey = `${productType}-${product.id}`;
                return (
                  <div key={productKey} className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition flex flex-col">
                    ...
                    <div className="grid grid-cols-2">
                      ...
                      <p className="text-sm text-gray-600">Price: ₹{parseFloat(product.price).toFixed(2)}</p>
                      <p className="text-sm text-gray-600">Discount: {parseFloat(product.discount).toFixed(2)}%</p>
                      <p className="text-sm text-gray-600">Stock: {product.stock} {product.per}</p>
                      <div className="mt-2 flex items-center justify-between">
                        <label className="inline-flex items-center">
                          ...
                          />
                          <div className={`w-11 h-6 rounded-full ${toggleStates[productKey] ? 'bg-green-600' : 'bg-red-600'}`}>
                            <div className={`w-5 h-5 translate-y-[2px] bg-white rounded-full transition-transform ${toggleStates[productKey] ? 'translate-x-5' : 'translate-x-0.5'}`}></div>
                          </div>
                        </label>
                      </div>
                    </div>
                    <div className="mt-4 flex justify-between gap-2">
                      <button
                        ...
                        className="flex-1 bg-blue-600 text-white px-2 py-1 rounded-md text-sm hover:bg-blue-700 flex items-center justify-center gap-1"
                      >
                        <FaEye className="h-4 w-4" /> View
                      </button>
                      <button
                        onClick={() => openModal(true, product)}
                        className="flex-1 bg-yellow-600 text-white px-2 py-1 rounded-md text-sm hover:bg-yellow-700 flex items-center justify-center gap-1"
                      >
                        <FaEdit className="h-4 w-4" /> Edit
                      </button>
                      <button
                        onClick={() => handleDelete(product)}
                        className="flex-1 bg-red-600 text-white px-2 py-1 rounded-md text-sm hover:bg-red-700 flex items-center justify-center gap-1"
                      >
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex justify-center space-x-2">
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm ${currentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
              >
                Previous
              </button>
              {[...Array(totalPages).keys()].map(page => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm ${currentPage === page + 1 ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border'}`}
                >
                  {page + 1}
                </button>
              ))}
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm ${currentPage === totalPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
              >
                Next
              </button>
            </div>
          ...
          >
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              <h2 className="text-xl font-semibold text-gray-900 mb-6 text-center">Add New Product</h2>
              <FormFields
                isEdit={false}
                formData={formData}
          ...
          >
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              <h2 className="text-xl font-semibold text-gray-900 mb-6 text-center">Edit Product</h2>
              <FormFields
                isEdit={true}
                formData={formData}
                handleInputChange={handleInputChange}
          ...
          >
            {selectedProduct && (
              <div className="bg-white rounded-lg p-6 max-w-md w-full">
                <h2 className="text-xl font-bold text-gray-900 mb-6 text-center">Product Details</h2>
                  ...
                  <div className="grid grid-cols-1 gap-2 text-sm">
                    {['product_type', 'serial_number', 'productname', 'price', 'per', 'discount', 'stock', 'status'].map(field => (
                      <div key={field} className="flex">
                        <span className="font-medium text-gray-700 w-24">{capitalize(field) + ':'}</span>
