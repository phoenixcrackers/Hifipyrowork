...
export default function PendingPayments() {
  ...
  const [selectedBankAccount, setSelectedBankAccount] = useState(''); // Single bank account selection
  const [bankAccounts, setBankAccounts] = useState([]); // Dynamically fetched bank accounts
  ...
  const currentBookings = pendingBookings.slice(indexOfFirstItem, indexOfLastItem);
    ...
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="flex-1 flex items-top justify-center onefifty:ml-[0%] hundred:ml-[1%]">
        <div className="w-full max-w-5xl p-6">
          <h1 className="text-4xl font-bold mb-8 text-center text-gray-800 dark:text-gray-100 mobile:text-2xl hundred:text-3xl">Pending Transactions</h1>
          {error && <div className="bg-red-100 dark:bg-red-900 p-2 mb-4 text-red-700 dark:text-red-300 mobile:text-sm hundred:text-base">{error}</div>}
          <div className="grid grid-cols-1 gap-4 hundred:gap-x-50 mobile:grid-cols-1 tab:grid-cols-2 hundred:grid-cols-4">
            {currentBookings.map(booking => (
              <div key={booking.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow hover:shadow-lg transition-shadow hundred:w-70 mobile:p-2 hundred:p-3">
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Order ID:</strong> {booking.order_id}</p>
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Customer:</strong> {booking.customer_name}</p>
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Total:</strong> Rs.{booking.total || '0.00'}</p>
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Paid:</strong> Rs.{booking.amount_paid || '0.00'}</p>
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Balance:</strong> Rs.{getBalance(booking).toFixed(2)}</p>
                <p className="text-gray-900 dark:text-gray-100 mobile:text-sm hundred:text-base"><strong>Status:</strong> {booking.status}</p>
            ...
            <div className="flex justify-center mt-4">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                  ...
                  className={`mx-1 px-3 py-1 rounded ${currentPage === page ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100'} hover:bg-blue-700 mobile:text-xs hundred:text-sm mobile:px-2 mobile:py-1 hundred:px-3 hundred:py-1`}
                >
                  {page}
                </button>
              ))}
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedBankAccount(''); // Reset bank account when admin changes
              }}
              className="p-2 border border-gray-300 dark:border-gray-600 mb-4 w-full bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            >
