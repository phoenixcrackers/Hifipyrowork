...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [prodRes, statesRes] = await Promise.all([
          fetch(`${API_BASE_URL}/api/product`),
          fetch(`${API_BASE_URL}/api/locations/states`)
        ]);
        const [products, states] = await Promise.all([prodRes.json(), statesRes.json()]);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      {(state.isCartOpen || state.showForm) && <div className="fixed inset-0 bg-black/40 z-30" onClick={() => setState(s => ({ ...s, isCartOpen: false, showForm: false }))} />}
      {state.showSuccess && (
      ...
      <main className="relative pt-5 mobile:px-5 mobile:pt-2 max-w-7xl mx-auto">
        ...
        <div className="mt-12 mb-10">
          <h2 className="text-3xl text-sky-500 mb-5 font-semibold capitalize border-b-4 border-sky-500 pb-2">Gift Box Dealers</h2>
      ...
      {state.showForm && (
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.3 }} className="fixed inset-0 flex items-center justify-center z-50">
          <div className="w-full max-w-md mx-4 p-6 rounded-xl" style={styles.modal}>
            <h2 className="text-lg font-bold text-sky-800 mb-4">Customer Details</h2>
            <form onSubmit={handleFormSubmit} className="space-y-4">
              {[
                ...
                { label: "District", key: "district", type: "select", options: state.districts, required: true, disabled: !state.customer.state },
                ...
                <div key={key}>
                  ...
                  {type === "select" ? (
                    ...
                    <textarea value={state.customer[key]} onChange={e => handleInputChange(key, e.target.value)} placeholder={placeholder} className="w-full p-2 rounded-md mt-1" style={styles.input} required={required} />
                  ) : (
                    <input type={type} value={state.customer[key]} onChange={e => handleInputChange(key, e.target.value)} placeholder={placeholder} className="w-full p-2 rounded-md mt-1" style={styles.input} required={required} />
                  )}
                </div>
              ))}
              <div className="flex gap-2">
                <button type="button" onClick={() => setState(s => ({ ...s, showForm: false }))} className="flex-1 text-white text-sm font-semibold py-3 rounded-xl" style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.9), rgba(220,38,38,0.9))", boxShadow: "0 5px 15px rgba(239,68,68,0.3)" }}>Cancel</button>
                <button type="submit" className="flex-1 text-white text-sm font-semibold py-3 rounded-xl" style={styles.button}>Confirm Booking</button>
              </div>
            </form>
          </div>
        </motion.div>
